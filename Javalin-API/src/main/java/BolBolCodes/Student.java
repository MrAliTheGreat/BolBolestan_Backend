...
public class Student {
    ...
    private ArrayList<PassedCourse> studentPassedCourses = new ArrayList<PassedCourse>();


    public Student(String studentId , String name , String secondName , String birthDate){
    ...
    private PassedCourse findPassedCourseFromCode(String code){
    ...
    private boolean checkHasPassedPrerequisites(ArrayList<String> prerequisites){
        PassedCourse foundPreCourse;
        for(String preCode : prerequisites){
            foundPreCourse = findPassedCourseFromCode(preCode);
            if(Objects.isNull(foundPreCourse)){
                return false;
            }
            if(foundPreCourse.getGrade() < 10){
                return false;
            }
        }
        return true;
    ...
    private boolean checkSingleCourseTimeWithOthers(Offering offer){
        for(Offering otherOffer : this.studentCourses){
    ...
    private boolean checkSingleExamTimeWithOthers(Offering offer){
        for(Offering otherOffer : this.studentCourses){
...
//                System.out.println(resultMessage);
        ...
        if(!checkHasPassedPrerequisites(offer.getPrerequisites())){
            return false;
        }
...
//        System.out.println(resultMessage);
    ...
    public boolean removeFromWeeklyScheduleOfStudent(String code , String classCode){
        ...
        for(int i = 0 ; i < studentCourses.size() ; i++){
            if(studentCourses.get(i).getCode().equals(code) && studentCourses.get(i).getClassCode().equals(classCode)){
...
//        System.out.println(resultMessage);
    ...
    public boolean addToPassedCourses(PassedCourse passedCourse){
        if(!Objects.isNull(findPassedCourseFromCode(passedCourse.getCode()))){
            JSONObject resultMessage = new JSONObject();
            resultMessage.put("success" , false);
...
//        System.out.println(resultMessage);
    ...
    public int calculateTotalUnitsOffers(){
        int numTotalUnits = 0;
        for(Offering offer : this.studentCourses){
    ...
    public ArrayList<Offering> getStudentCourses() {
        return studentCourses;
    }
}
